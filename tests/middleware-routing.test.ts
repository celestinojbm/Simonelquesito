 ...
 * redirige a /login. Las APIs pasan, salvo las exclusivas del storefront
 * (/api/ubicacion/**, /api/webhooks/payments/sandbox), que responden 404
 * fail-closed. Con STOREFRONT_ENABLED="true" nada se gatea.
 */
...
const BLOCKED_APIS = ["/api/ubicacion/tok123", "/api/webhooks/payments/sandbox"];
...
describe.each(INTERNAL_VALUES)("middleware — modo interno (STOREFRONT_ENABLED %s)", (_label, value) => {
  ...
  it("las APIs exclusivas del storefront responden 404 (fail-closed, sin redirect)", () => {
    stub();
...
describe("middleware — storefront HABILITADO (STOREFRONT_ENABLED='true')", () => {
  ...
  it("no gatea las APIs del storefront", () => {
    vi.stubEnv("STOREFRONT_ENABLED", "true");
    for (const p of BLOCKED_APIS) expect(isNext(run(p)), `api habilitada ${p}`).toBe(true);
  });
});
